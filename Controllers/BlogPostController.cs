...
namespace blogapp.Controllers
{
    public class BlogPostController : Controller
    {
        ...
        public IActionResult Create()
        {
            ...
            // For now, allow all logged-in users to create posts (temporary for testing)
            // TODO: Re-enable blog access check when ready
            /*
            // Check if user has blog access
            if (user.BlogAccessStatus != AccessStatus.Approved)
            {
                // User is logged in but doesn't have blog access
                if (user.BlogAccessStatus == AccessStatus.None)
                {
                    // First time user, redirect to become creator
                    return RedirectToAction("BecomeCreator", "User");
                }
                else if (user.BlogAccessStatus == AccessStatus.Pending)
                {
                    // User has pending request
                    TempData["Message"] = "Your blog creator request is pending approval. Please wait for admin approval.";
                    return RedirectToAction("Dashboard", "User");
                }
                else if (user.BlogAccessStatus == AccessStatus.Rejected)
                {
                    // User's request was rejected
                    TempData["Message"] = "Your blog creator request was rejected. Please contact admin for more information.";
                    return RedirectToAction("Dashboard", "User");
                }
            }
            */
